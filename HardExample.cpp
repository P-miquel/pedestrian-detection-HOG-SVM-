...
int main()
{
	...
	string ImgName;
	ifstream fin_detector("HOGDetectorForOpenCV.txt");//打开自己训练的SVM检测器文件  
	ifstream fin_imgList("NegtiveName.txt");//打开原始负样本图片文件列表  
	...
	while (getline(fin_imgList, ImgName))
	{
		...
		string fullName = "F:\\Zhou\\GetPicture\\GetPicture\\NegtiveSampleInit\\" + ImgName;//加上路径名  
		...
		for (int i = 0; i < found.size(); i++)
		{
			...
			sprintf_s(saveName, "hardexample%09d.bmp", hardExampleCount++);//生成hard example图片的文件名  
