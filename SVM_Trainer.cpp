...
#define PosSamNO 1102 //2416    //正样本个数  
#define NegSamNO 5020    //负样本个数  
...
int main()
{
	...
	if (TRAIN)
	{
		...
		ifstream finPos("PostiveName.txt");//正样本图片的文件名列表
		//ifstream finPos("PersonFromVOC2012List.txt");//正样本图片的文件名列表
		ifstream finNeg("NewNegPic.txt");//负样本图片的文件名列表
		...
		for (int num = 0; num < PosSamNO && getline(finPos, ImgName); num++)
		{
			...
			ImgName = "F:\\Zhou\\GetPicture\\GetPicture\\PositiveSample\\" + ImgName;//加上正样本的路径名
		...
		{
			cout << "处理：" << ImgName << endl;
			ImgName = "F:\\Zhou\\GetNegPic\\GetNegPic\\NegtivePic\\" + ImgName;//加上负样本的路径名
			Mat src = imread(ImgName);//读取图片
			//resize(src,src,Size(64,128));
		...
		if (HardExampleNO > 0)
		{
			...
			for (int num = 1; num < HardExampleNO && getline(finHardExample, ImgName); num++)
			{
				cout << "处理：" << ImgName << endl;
				ImgName = "F:\\Zhou\\HardExample\\HardExample\\HardExample\\" + ImgName;//加上HardExample负样本的路径名
				Mat src = imread(ImgName);//读取图片
		...
		cout << "训练完成" << endl;
		svm.save("SVM_HOG_NewTrain.xml");//将训练好的SVM模型保存为xml文件
	}
	else //若TRAIN为false，从XML文件读取训练好的分类器
	{
		svm.load("SVM_HOG_NewTrain.xml");//从XML文件读取训练好的SVM模型
	}
